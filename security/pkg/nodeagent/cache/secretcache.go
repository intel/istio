...
func (s *secretCache) GetWorkloadCertChain() (certificateChain []byte) {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.workload.CertificateChain
}
...
// used when sgxEnabled
func (s *secretCache) SetWorkloadCert(value []byte) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.sgxCertReady = false
	s.workload.RootCert = value
	s.workload.CertificateChain = value
	s.workload.CreatedTime = time.Time{}
	s.workload.ExpireTime = time.Time{}
}
...
func (sc *SecretManagerClient) VerifyCSR(csr []byte) error {
	_, err := pkiutil.ParsePemEncodedCSR(csr)
	return err
}

func (sc *SecretManagerClient) UpdateCSR(csr []byte) (needPush bool, certReady bool, retErr error) {
	sc.cache.sgxMutex.Lock()
	defer sc.cache.sgxMutex.Unlock()

	if err := sc.VerifyCSR(csr); err != nil {
		cacheLog.Warnf("New csrByte is invalid and will be ignored: %s", err.Error())
		return false, false, errors.New("csrByte is invalid")
	}
	cacheLog.Info("New csrByte is valid")
	ret := bytes.Compare(sc.cache.csrBytes, csr)
	if ret == 0 && sc.cache.sgxCertReady {
		cacheLog.Info("New csrByte is the same as the one in cache")
		return false, false, nil
	}
	sc.cache.csrBytes = csr
	cacheLog.Info("csrByte has been updated by UpdateCSR()")

	ns, err := sc.generateNewSecretbyExternalCSR(security.WorkloadKeyCertResourceName)
	if err != nil {
		cacheLog.Warnf("generateNewSecretbyExternalCSR() error: %s", err.Error())
		return false, false, err
	}
	sc.registerSecret(*ns, true, true)
	cacheLog.Info("New certificate has been generated by using csrByte")
	sc.cache.sgxCertReady = true
...
func (sc *SecretManagerClient) getCachedSecret(resourceName string) (secret *security.SecretItem) {
	...
	// only when SecretItem.CertificateChain is not nil can get cert form the cache
	if c := sc.cache.GetWorkload(); c != nil {
		if certchain := sc.cache.GetWorkloadCertChain(); certchain != nil {
			...
			cacheLog.Warnf("Ceritificate in cache overtime, needs cert rotation")
			return nil
		}
		return ns
	}
...
func (sc *SecretManagerClient) GenerateSecret(resourceName string) (secret *security.SecretItem, err error) {
	...
	if sc.cache.GetWorkload() != nil && sc.cache.GetWorkloadCertChain() == nil {
		ns, err = sc.generateNewSecretbyExternalCSR(resourceName)
		if err != nil {
			return nil, fmt.Errorf("failed to regenerate workload certchain: %v", err)
		}
		sc.cache.sgxCertReady = true
		cacheLog.Debugf("generate secret %q by customCSR, sgxCertReady", resourceName)
	} else {
		// send request to CA to get new workload certificate
	...
	// When sgx is not enabled,run as normal
	if sc.configOptions.SgxEnabled {
		if sc.cache.sgxCertReady {
			// For sgx cert rotation
			sc.registerSecret(*ns, true, true)
		} else {
			// Generate the first key pair in momory, but not in sgx enclave
			sc.registerSecret(*ns, false, false)
		}
	} else {
		// When sgx is not enabled,run as normal
		sc.registerSecret(*ns, false, true)
	}
...
func (sc *SecretManagerClient) generateNewSecretbyExternalCSR(resourceName string) (*security.SecretItem, error) {
	trustBundlePEM := []string{}
	var rootCertPEM []byte
	if sc.caClient == nil {
		return nil, fmt.Errorf("attempted to fetch secret, but ca client is nil")
	...
	// Generate the cert/key, send CSR to CA.
	if len(sc.cache.csrBytes) == 0 {
		return nil, fmt.Errorf("csrBytes is nil, cannot generate cert")
	}

	numOutgoingRequests.With(RequestType.Value(monitoring.CSR)).Increment()
	timeBeforeCSR := time.Now()
	certChainPEM, err := sc.caClient.CSRSign(sc.cache.csrBytes, int64(sc.configOptions.SecretTTL.Seconds()))
	if err == nil {
		trustBundlePEM, err = sc.caClient.GetRootCertBundle()
	}
	csrLatency := float64(time.Since(timeBeforeCSR).Nanoseconds()) / float64(time.Millisecond)
...
func (sc *SecretManagerClient) registerSecret(item security.SecretItem, force bool, rotate bool) {
	...
	sc.queue.PushDelayed(func() error {
		...
		// if sgxEnable just regenerate cert again
		if sc.configOptions.SgxEnabled {
			sc.cache.SetWorkloadCert(nil)
		} else {
